...
export default function PostTable({ posts: propPosts, initialSearchTerm = "" }: PostTableProps) {
  ...
  const [sortBy, setSortBy] = useState<'likes' | 'dislikes' | 'date' | 'reports'>('date');
  ...
  const filteredPosts = displayPosts.filter(post => {
    ...
    const getValue = (post: Post, field: string) => {
      switch (field) {
        ...
        case 'dislikes': return post.dislikes;
        case 'reports': return post.reportCount;
        case 'date': return new Date(post.postDate).getTime();
        default: return 0;
      }
    };
  ...
  const handleWarn = useCallback((postId: string) => {
    const post = filteredPosts.find(p => p.id === postId);
    if (post) {
  ...
  const handleNotificationSent = useCallback(async (notificationId: string) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
          ...
          <thead className="bg-gray-50 dark:bg-gray-700">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                <button onClick={() => handleSort('date')} className="flex items-center space-x-1 hover:text-purple-600">
                  <span>Post Date</span>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                <button onClick={() => handleSort('likes')} className="flex items-center space-x-1 hover:text-purple-600">
                  <span>Likes</span>
                  <span>{getSortIcon('likes')}</span>
                </button>
              </th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                <button onClick={() => handleSort('dislikes')} className="flex items-center space-x-1 hover:text-purple-600">
                  <span>Dislikes</span>
                  <span>{getSortIcon('dislikes')}</span>
                </button>
              </th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                <button onClick={() => handleSort('reports')} className="flex items-center space-x-1 hover:text-purple-600">
                  <span>Reports</span>
                  <span>{getSortIcon('reports')}</span>
                </button>
              </th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                Status
      ...
      <div className="bg-white dark:bg-gray-800 px-4 py-3 border-t border-gray-200 dark:border-gray-700 sm:px-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center text-sm text-gray-700 dark:text-gray-300">
            ...
            <span>{startIndex + 1}-{Math.min(startIndex + rowsPerPage, filteredPosts.length)} of {filteredPosts.length} rows</span>
          </div>
          <div className="flex items-center space-x-2">
            <button 
            ...
            <div className="bg-white dark:bg-zinc-800 rounded-lg shadow-xl max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto">
              <div className="p-4 border-b border-zinc-200 dark:border-zinc-700">
                ...
                <p className="text-sm text-zinc-600 dark:text-zinc-400 mt-1">
                  Reported User: {selectedPostForNotification.user.name} ({selectedPostForNotification.user.id})
                </p>
              </div>
              <div className="p-4">
      ...
      {showDeleteModal && postToDelete && (
        <>
          {/* Modal backdrop */}
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-60" onClick={cancelRemove} />
              ...
              <div className="p-6">
                ...
                <div className="mb-6">
                  <p className="text-sm text-gray-700 dark:text-gray-300 mb-3">
                    Are you sure you want to remove this post by <strong>{postToDelete.user.name}</strong>?
                  </p>
                  
                  <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 mb-4">
                    <p className="text-xs text-gray-600 dark:text-gray-400 mb-1">Post content:</p>
