...
import { updatePostStatus } from "@/lib/firestore/posts";
import { updatePostWarningStatus } from "@/lib/firestore/posts";
...
export default function ReportTable({
  ...
  const [selectedReportForDetail, setSelectedReportForDetail] =
  ...
  const [reportToRemoveWarn, setReportToRemoveWarn] = useState<Report | null>(
    null,
  );
  ...
  const reportCountByPostId = reports.reduce<Record<string, number>>(
    (acc, r) => {
  ...
  const handleSort = (field: "reportCount") => {
    if (sortBy === field) {
  ...
  const confirmDismissReport = async () => {
    ...
    try {
      ...
      const relatedReports = reports.filter(
        (r) => r.postId === reportToDismiss.postId,
      );
      await Promise.all(
        relatedReports.map((r) => updateStatus(r.id, "dismissed")),
      );
      ...
      <div className="p-6 border-b border-gray-200 dark:border-gray-700">
        <div className="flex flex-col space-y-4">
          ...
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search by reporter, reported user, content or post ID..."
              value={searchTerm}
        ...
        <table className="w-full table-fixed">
          ...
          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            {paginatedReports.map((report) => {
              const isHighlighted =
                highlightPostId && report.postId === highlightPostId;
                ...
                >
                  ...
                  <td className="px-3 py-4 text-sm text-gray-900 dark:text-white">
                    <span className="flex items-center">
                      <span
                        className={`w-2 h-2 rounded-full mr-2 shrink-0 ${getReportCountColor(reportCountByPostId[report.postId] || report.reportCount)}`}
                      ></span>
                  ...
                  <td className="px-3 py-4 text-sm">
                    <div className="flex items-center space-x-1">
                      ...
                      {report.status === "pending" &&
                        !warnedPostIds.has(report.postId) && (
                          <button
                            onClick={() =>
                              handlePostRemovalAndWarning(report.id)
                            }
          ...
          <div className="bg-white dark:bg-zinc-800 rounded-lg max-w-md w-full max-h-[90vh] overflow-y-auto">
            <div className="p-4 border-b border-zinc-200 dark:border-zinc-700">
              ...
              <p className="text-sm text-zinc-600 dark:text-zinc-400 mt-1">
                Reported User: {selectedReport.reported.name} (
            ...
            <div className="p-4">
              <SendNotificationModal
                ...
                onNotificationSent={(notificationId) => {
                  toast.success(
                    `Notification sent successfully! ID: ${notificationId}`,
                  );
                  if (selectedReport) {
                    setWarnedPostIds((prev) =>
                      new Set(prev).add(selectedReport.postId),
                    );
                  }
              ...
              <div className="p-6">
                ...
                <div className="mb-6">
                  <p className="text-sm text-gray-700 dark:text-gray-300 mb-3">
                    ...
                    and{" "}
                    <strong>
                      warn the user {reportToRemoveWarn.reported.name}
                    </strong>
                    ?
                  </p>
              ...
              <div className="p-6">
                ...
                <div className="mb-6">
                  <p className="text-sm text-gray-700 dark:text-gray-300 mb-3">
                    ...
                    indicates the report was invalid or does not require action.
                  </p>
                  ...
                  <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 mb-4">
                    ...
                    <p className="text-xs text-gray-600 dark:text-gray-400 mt-2 mb-1">
                      Reporter:
                    </p>
