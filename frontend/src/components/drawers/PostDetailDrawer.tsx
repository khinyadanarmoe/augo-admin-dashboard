...
import { incrementUserWarningCount, fetchUserById, updateUserStatus } from '@/lib/firestore/users';
...
export default function PostDetailDrawer({ post, isOpen, onClose, onWarn, onRemove }: PostDetailDrawerProps) {
  ...
  const confirmWarn = async () => {
    ...
    try {
      ...
      console.log('Warning notification sent to user and warning count incremented:', post.user.id);
      setShowConfirmModal(false);
    } catch (error) {
      console.error('Error sending warning notification or incrementing warning count:', error);
  ...
  return (
    <>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Reports
                  {shouldShowWarning && (
                    <span className="ml-1 text-xs text-red-600 font-medium">⚠️</span>
                  )}
                </label>
            ...
            {shouldShowWarning && (
              <div className="bg-orange-50 dark:bg-orange-900/50 border border-orange-200 dark:border-orange-800 rounded-lg p-4">
                <div className="flex items-start space-x-3">
                  <svg className="w-5 h-5 text-orange-600 dark:text-orange-400 mt-0.5 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-orange-700 dark:text-orange-400 mt-1">
                      This post has reached the maximum threshold ({config?.reportThresholds?.urgent || 10
                      } reports) and requires administrative action.
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-3 pt-4 border-t border-gray-200 dark:border-gray-700">
            {canWarn && (
              ...
              >
                ...
                <span>
                  {shouldShowWarning ? 'Warn User (Threshold Exceeded)' : 'Warn the User'}
                </span>
              </button>
            )}
      ...
      {showConfirmModal && (
        <>
          {/* Modal backdrop */}
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-60" onClick={cancelWarn} />
              ...
              <div className="p-6">
                ...
                <div className="mb-6">
                  <p className="text-sm text-gray-700 dark:text-gray-300 mb-3">
                    Are you sure you want to warn <strong>{post.user.name}</strong> for this post?
                  </p>
          ...
          <div className="fixed inset-0 z-70 flex items-center justify-center p-4">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full mx-4">
              <div className="p-6">
                ...
                <div className="mb-6">
                  <p className="text-sm text-gray-700 dark:text-gray-300 mb-3">
                    Are you sure you want to remove this post by <strong>{post.user.name}</strong>?
                  </p>
